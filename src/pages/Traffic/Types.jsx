...
const Samples = props => {
  const { trafficTypes, user } = props
  const userId = user.id
  useEffect(() => {
    const topbar = {
      title: 'Tipos camiones',
      menu: {
        Trafico: '/traffic',
  ...
  const editable = {
    onRowAdd: newData =>
      new Promise((resolve, reject) => {
        setTimeout(() => {
          ...
          props
            .create(`traffic/types`, 'CREATE_SAMPLE', newData)
            .then(() =>
              props.getAll('traffic/types', 'GET_TRAFFIC_TYPES')
            )
            .then(() => resolve())
    ...
    onRowDelete: oldData =>
      new Promise((resolve, reject) => {
        setTimeout(() => {
          props
            .deleted(`traffic/types/${oldData.id}`, 'DELETE_SAMPLE')
            .then(() =>
              props.getAll('traffic/types', 'GET_TRAFFIC_TYPES')
            )
            .then(() => resolve())
