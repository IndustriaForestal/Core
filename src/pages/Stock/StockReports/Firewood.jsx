...
const Nails = props => {
  const { stockReportFirewood } = props
  ...
  useEffect(() => {
    ...
    setWraper(true)
    props.getAll(
      'stock/reports/firewood',
      'GET_STOCK_REPORT_FIREWOOD'
    )
  ...
  if (stockReportFirewood) {
    const dataIfisa1 = stockReportFirewood.raws
    ...
    const dataIfisa2 = stockReportFirewood.raws
    ...
    const dataIfisa1Wood = stockReportFirewood.rawsWood
      .filter(item => item.plant_id === 1)
      .map(pallet => ({
        Especie: pallet.name,
        'P/T': pallet.pt,
    ...
    const dataIfisa2Wood = stockReportFirewood.rawsWood
      .filter(item => item.plant_id === 2)
      .map(pallet => ({
        Especie: pallet.name,
        'P/T': pallet.pt,
    ...
    return (
      <>
        ...
        <div>
          <CSVLink data={dataIfisa1} filename={'ifisa1_lenia.csv'}>
            <Button>Reporte IFISA 1</Button>
          </CSVLink>
          <CSVLink data={dataIfisa2} filename={'ifisa2_lenia.csv'}>
            <Button>Reporte IFISA 2</Button>
          </CSVLink>
          <CSVLink
            data={dataIfisa1Wood}
            filename={'ifisa1_lenia_especie.csv'}
          >
            <Button>Reporte IFISA 1 por especie</Button>
          </CSVLink>
          <CSVLink
            data={dataIfisa2Wood}
            filename={'ifisa2_lenia_especie.csv'}
          >
...
const mapStateToProps = state => {
  return {
    stockReportFirewood: state.reducerStock.stockReportFirewood,
  }
}
